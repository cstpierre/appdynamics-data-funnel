...
public class EventsFunnel implements FunnelInterface, Runnable
{
	...
	public void initialize(String funnelName, String outputType, long startEpoch, long endEpoch, ControllerConfiguration conf, Element funnelDefinition) throws AppDFunnelInstantiationError 
	{
		...
		Node eventtypesNode = funnelDefinition.selectSingleNode( "eventtypes" );
		if (eventtypesNode == null)
		{
			throw new AppDFunnelInstantiationError("Unable to retrieve a eventtypes node under this funnel definition. Cannot continue.");
		}
		else
		{
			String nodeVal = eventtypesNode.getText();
			if ( nodeVal.isEmpty() )
			{
				throw new AppDFunnelInstantiationError("Value for evettypes must not be empty. Cannot continue.");
			}
			eventtypes = nodeVal;
		}
		
		Node severitiesNode = funnelDefinition.selectSingleNode( "severities" );
		if (severitiesNode == null)
		{
			throw new AppDFunnelInstantiationError("Unable to retrieve a severities node under this funnel definition. Cannot continue.");
		}
		else
		{
			String nodeVal = severitiesNode.getText();
			if ( nodeVal.isEmpty() )
			{
				throw new AppDFunnelInstantiationError("Value for severities must not be empty. Cannot continue.");
			}
			severities = nodeVal;
		}
	...
	public String getRESTURL() throws AppDFunnelExecutionError 
	{
		...
		try
		{
			...
			builder.append("/events?event-types=");
			builder.append(java.net.URLEncoder.encode(eventtypes, "ISO-8859-1"));
			builder.append("&severities=");
			builder.append(java.net.URLEncoder.encode(severities, "ISO-8859-1"));
	...
	{
		//System.out.println("Executing Events Funnel: " + name);
