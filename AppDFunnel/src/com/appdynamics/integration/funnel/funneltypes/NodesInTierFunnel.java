...
public class NodesInTierFunnel implements FunnelInterface, Runnable 
{
	...
	public void initialize(String funnelName, String outputType, long startEpoch, long endEpoch, ControllerConfiguration conf, Element funnelDefinition) throws AppDFunnelInstantiationError 
	{
		...
		Node tieridNode = funnelDefinition.selectSingleNode( "tierid" );
		if (tieridNode == null)
		{
			throw new AppDFunnelInstantiationError("Unable to retrieve a tierid node under this funnel definition. Cannot continue.");
		}
		else
		{
			String nodeVal = tieridNode.getText();
			if ( nodeVal.isEmpty() )
			{
				throw new AppDFunnelInstantiationError("Value for tierid must not be empty. Cannot continue.");
			}
			tierid = nodeVal;
		}
	...
	public String getRESTURL() throws AppDFunnelExecutionError 
	{
		...
		try
		{
			...
			builder.append(java.net.URLEncoder.encode(tierid, "ISO-8859-1"));
			builder.append("/nodes?output=");
	...
	{
		//System.out.println("Executing NodesInTier Funnel: " + name);
